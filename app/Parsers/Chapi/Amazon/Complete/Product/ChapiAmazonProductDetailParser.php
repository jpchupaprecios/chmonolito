...
namespace App\Parsers\Chapi\Amazon\Complete\Product;
...
final class ChapiAmazonProductDetailParser
{
     ...
     */
    public function parse($html, string $vendor, string $productId, string $cookie): \stdClass
    {
        $this->product = new \stdClass();

        if (isset($html['result']) && is_array($html['result'])) {
            Log::debug('$html[result] is array');
            Log::debug('$html[result] value: ' . print_r( $html['result'], true ));
            return $this->product;
        }
        ...
        $this->product->shipping_price = $delivery;
        $this->product->score = $this->getScore();
        $this->product->rating = $this->getRating();
        ...
        $this->product->thumbnails = $this->getThumbnails($thumbnails);
    ...
    private function getThumbnails(array $thumbs): array
    {
        ...
        foreach ($thumbs as $image) {
            $thumbnails[] = new Thumbnail([
                'link' => $image,
                'product_details_id' => $this->product->product_id,
            ]);
        }
...
//            'https://www.amazon.com/gp/product/ajax/ref=dp_aod_unknown_mbc?asin=' . $productId . '&m=&qid=&smid=&sourcecustomerorglistid=&sourcecustomerorglistitemid=&sr=&pc=dp&experienceId=aodAjaxMain&condition=new',
            'https://www.amazon.com/gp/product/ajax/ref=tmm_dvd_new_aod_0?filters=%257B%2522all%2522%253Atrue%252C%2522new%2522%253Atrue%257D&condition=new&asin=' . $productId . '&pc=dp&experienceId=aodAjaxMain',
            $this->cookie,
            false
        );
    ...
    private function getCategories(string $vendor): array
    {
        ...
        if ($categoriesLi) {
            foreach ($categoriesLi as $categorieLi) {
                ...
                $category->link = $href;
                $category->name = $title;
                $category->product_details_id = $this->product->product_id;
                $categories[] = $category;
            }
        }
